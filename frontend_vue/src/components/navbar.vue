  ...
  <div class="navbar navbar-dark bg-dark mb-5">
      <div class="d-flex flex-row px-4">
        ...
        <router-link 
          to="Dashboard" 
          v-if="isLoggedIn" 
          class="navbar-brand">Dashboard
        </router-link>
        <router-link 
          to="MyComplaints" 
          v-if="isLoggedIn && ['student', 'admin', 'staff'].includes(userRole)" 
          class="navbar-brand">My Complaints
        </router-link>
        <router-link 
          to="MyMaintreqs" 
          v-if="isLoggedIn && ['student', 'admin', 'staff', 'technician'].includes(userRole)" 
          class="navbar-brand">My Maintenance Requests
        </router-link>
        <router-link 
          to="MyFoodorders" 
          v-if="isLoggedIn && ['student', 'admin', 'staff', 'chef'].includes(userRole)" 
          class="navbar-brand">My Food Orders
        </router-link>
        <router-link 
          to="MyPackages" 
          v-if="isLoggedIn && ['student', 'admin', 'staff'].includes(userRole)" 
          class="navbar-brand">My Packages
        </router-link>
      </div>
